...
function filter() {
    ...
    if(ip!="any" && name_entity!="any" && port!="any"){ //apenas IP e porta
        ...
        if(array_ip_porta.length == 0) {
            alert("Não existem tráfegos com esses parâmetros.");
            return;
        }
    ...
    if(animar.length == 0) alert("Não existem tráfegos com esses parâmetros..")
...
function filtraPorNome(nome){
    ...
    if(nodes.length==0){
        window.alert("Não existem nós com esse nome");
        return;
    }
...
function filtraPorIP(ip) {
    ...
    if (no_ip == "") {
        window.alert("Não existem nós com esse IP");
        return;
    }
